...
export default function ResultsPage() {
  ...
  const [playerName, setPlayerName] = useState("");
  const [playerAge, setPlayerAge] = useState(0);
  const [dyslexicStatus, setDyslexicStatus] = useState<
    "dyslexic" | "notdyslexic" | null
  >(null);
  const [gameResults, setGameResults] = useState<any>({});
  const [loading, setLoading] = useState(true);
  const [overallScore, setOverallScore] = useState(0);
  ...
  const saveResults = async () => {
    ...
    if (dyslexicStatus === null) {
      console.error("Dyslexic status not selected");
      return;
    }
  ...
  const handleShowResults = () => {
    if (dyslexicStatus === null) {
      alert(
        "Please select whether you are dyslexic or not before viewing results"
      );
      return;
    }
    setShowResults(true);
      ...
      <div className="min-h-screen bg-gradient-to-b from-cyan-500 to-blue-900 flex flex-col items-center justify-center p-4">
        <Card className="max-w-3xl w-full border-4 border-cyan-300 bg-white/90 backdrop-blur-sm shadow-xl">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-xl">
              Please select your dyslexic status to view your results
            </CardDescription>
          </CardHeader>

          <CardContent className="space-y-8 p-6">
            <div className="flex flex-col items-center gap-2 mb-4">
              <label className="font-medium text-lg">Select player type:</label>
              <div className="flex gap-4">
                <Button
                  variant={
                    dyslexicStatus === "dyslexic" ? "default" : "outline"
                  }
                  onClick={() => setDyslexicStatus("dyslexic")}
                ...
                <Button
                  variant={
                    dyslexicStatus === "notdyslexic" ? "default" : "outline"
                  }
                  onClick={() => setDyslexicStatus("notdyslexic")}
                  className="min-w-[120px]"
                >
                  Non-Dyslexic
                </Button>
